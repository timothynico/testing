...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <div id="requestType" class="btn-group w-100" role="group">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6 col-md-4">
                            ...
                            <select class="form-select form-select-sm" id="usageSelect" required>
                                ...
                                @forelse ($usageOptions as $option)
                                    <option value="{{ $option['usage'] }}" data-pallet="{{ $option['pallet_type'] }}"
                                        data-size="{{ $option['size'] }}" data-color="{{ $option['color'] }}"
                                        data-grade="{{ $option['grade'] }}">
                                        {{ $option['label'] ?: $option['usage'] }}
                                    </option>
                                @empty
                                    <option value="" disabled>{{ __('No usage available') }}</option>
                                @endforelse
                            </select>
                        </div>
                        ...
                        <div class="col-6 col-md-3 d-none" id="pullPalletTypeField">
                            ...
                            <select class="form-select form-select-sm" id="pullPalletType">
                                ...
                                @forelse ($palletTypeOptions as $option)
                                    <option value="{{ $option['pallet_type'] }}">
                                        {{ $option['pallet_type'] }}@if (!empty($option['size']))
                                            ({{ $option['size'] }})
                                            @endif@if (!empty($option['color']))
                                                - {{ $option['color'] }}
                                            @endif
                                    </option>
                                    @empty
                                        <option value="" disabled>{{ __('No pallet types available') }}</option>
                                    @endforelse
                                </select>
                            </div>
                            ...
                            <div class="col-6 col-md-4">
                                ...
                                <select class="form-select form-select-sm" id="warehouseFromSelect" required>
                                    ...
                                    @foreach ($custwh as $wh)
                                        <option value="{{ $wh->nidwh }}" data-customer="1"
                                            data-address="{{ $wh->calmtwh }}" data-nlat="{{ $wh->nlat }}"
                                            data-nlong="{{ $wh->nlong }}" data-nid="{{ $wh->nidwh }}"
                                            data-ckd="{{ $wh->ckdwh }}" data-city="{{ $wh->ckotawh }}">
                                            {{ $wh->cnmwh }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="col-6 col-md-4">
                                ...
                                <select class="form-select form-select-sm" id="warehouseToSelect" required>
                                    ...
                                    @foreach ($custwh as $wh)
                                        <option value="{{ $wh->nidwh }}" data-customer="1"
                                            data-nlat="{{ $wh->nlat }}" data-long="{{ $wh->nlong }}"
                                            data-address="{{ $wh->calmtwh }}" data-nid="{{ $wh->nidwh }}"
                                            data-ckd="{{ $wh->ckdwh }}" data-city="{{ $wh->ckotawh }}">
                                            {{ $wh->cnmwh }}
                                        </option>
                                    @endforeach
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center bg-success text-white">
                ...
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-sm btn-danger" id="resetAllBtn">
                    ...
                    <button type="button" class="btn btn-sm btn-light" id="openMailBtn">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <!-- Sender -->
                    <div class="col-12">
                        <label for="emailSender" class="form-label fw-semibold me-2">
                            {{ __('From: (Sender)') }}
                        </label>
                        <span
                            class="text-danger fw-semibold">{{ __('Use your registered email and double-check your selection!') }}</span>
                        <input type="text" class="form-control" id="emailSender" readonly value={{ $email }}>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const fromPlaceholderTemplate = warehouseFromSelect.options[0] ? warehouseFromSelect.options[0]
                    .cloneNode(true) : null;
                const toPlaceholderTemplate = warehouseToSelect.options[0] ? warehouseToSelect.options[0].cloneNode(
                    true) : null;
                ...
                function setWarehouseOptions(selectEl, optionType) {
                    ...
                    const placeholderTemplate = selectEl === warehouseFromSelect ? fromPlaceholderTemplate :
                        toPlaceholderTemplate;
                ...
                function sortSelectOptionsByDistance(selectEl, autoSelectSmallest = false) {
                    ...
                    const hasPreviousValue = previousValue && sortableOptions.some(option => option.value ===
                        previousValue);
                ...
                function setDistanceLabels(targetSelect, sourceSelect, autoSelectSmallest = false) {
                    ...
                    Array.from(targetSelect.options).forEach((targetOption, index) => {
                        ...
                        const baseName = targetOption.dataset.baseName || targetOption.textContent.split(' (')[
                            0].trim();
                        ...
                        if (!Number.isFinite(sourceLat) || !Number.isFinite(sourceLon) || !Number.isFinite(
                                targetLat) || !Number.isFinite(targetLon)) {
                        ...
                        const distanceLabel = roundedDistance === 0 ? '(Same city)' :
                            `(±${roundedDistance} Km)`;
                ...
                let isDistanceUpdateInProgress = false;

                function updateWarehouseDistanceLabels(autoSelectSmallest = false) {
                    if (isDistanceUpdateInProgress) {
                        return;
                    }
                    isDistanceUpdateInProgress = true;
                    ...
                    warehouseToStyledDropdown.sync();
                        ...
                        isDistanceUpdateInProgress = false;
                    }
                }
                ...
                async function fetchTokensIfNeeded(fromMeta, toMeta) {
                    ...
                    if ((!tokenFrom && fromKey) || (!tokenTo && toKey)) {
                        ...
                        try {
                            const resp = await fetch('/delivery/warehouse-token', {
                                method: 'POST',
                                headers: {
                ...
                function openMailClient() {
                    ...
                    const gmailUrl = `https://accounts.google.com/AccountChooser?continue=` +
                        encodeURIComponent(
                        ...
                        if (isTooLongForDesktop) {
                            ...
                            setTimeout(() => {
                                const userConfirmed = confirm(
                                    "We tried opening your mail app. If it didn't open, would you like to use Gmail in the browser instead?"
                                );
                                if (userConfirmed) {
                                    window.open(gmailUrl, '_blank');
                                }
                ...
                window.startGenerateRequestEmailTour = function() {
                    ...
                    const driverObj = driver({
                        ...
                        steps: [{
                            ...
                            {
                                element: '#palletVisualization',
                                popover: {
                                    title: '🧩 {{ __('Pallet Visualization') }}',
                            ...
                            {
                                element: '#warehouseFromSelectTrigger',
                                popover: {
                            ...
                            {
                                element: '#warehouseToSelectTrigger',
                                popover: {
